...
### [서비스 개요]
1. 누구나 쉽고 간단하게 도서관리 서비스를 개발할 수 있도록 사용이 간편하게 제작된 RESTful API
2. 
...
### [서비스 목적]
1. [기존 RESTful API 개발 경험](https://github.com/Blanc-et-noir/RestAPI)을 바탕으로 좀 더 체계적으로 구성된 서버를 구축해보는 것.
2. 기존의 뷰를 직접 구성하여 수행하는 시간적 부담이 상당한 테스트 방법 대신, Postman을 활용하여 자동화된 테스트 수행방법을 채택하고 익히는 것.
3. 뷰를 담당할 다른 개발자와 협업하여 두 시스템간의 통합을 이뤄보는 것.
...
### [서비스 특징]
1. CORS 기능을 지원하여 외부에서 별다른 설정없이 바로 사용 가능함.
2. 쿠키 대신 요청 및 응답헤더에 JWT Token을 첨부함으로써 
3. HTTPS 프로토콜을 사용하여 페이로드 및 헤더를 암호화함. (이전에 개발했던 서비스에서는 HTTP 프로토콜을 사용)
...
### [API 명세서]
1. [명세서 확인하기](https://documenter.getpostman.com/view/21751638/UzJETzE2)
